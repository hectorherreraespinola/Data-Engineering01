...
def startup():
    global DF
    DF = pd.read_csv('Datasets/peliculas_y_series.csv')

# Cargamos información sobre el proyecto
@app.get('/')
async def index():
    return {'Proyecto Individual Hector Herrera, de la carrera Data Science de Henry'}
...
async def get_count_plataform(platform:str):
    platform = platform.replace("'","")
    platform = platform.capitalize()
    Count_platform = DF[(DF.Plataform == platform)]  # Aplicamos una máscara de acuerdo al parámetro
...
async def get_listedin(genre:str):
    ...
    Plataformas, apariciones = list(DF.Plataform.unique()), list()   # Creamos una lista con las plataformas, y otra vacía
    for elem in Plataformas:    # Iteramos para cada plataforma
        DF_por_plataforma = DF[(DF.Plataform == elem)]   # Aplicamos una máscara de acuerdo al parámetro
...
async def get_actor(
    ...
    Cast_list = list(DF[(DF.Plataform == platform) & (DF.Release_Year == year)].Cast.fillna(''))
